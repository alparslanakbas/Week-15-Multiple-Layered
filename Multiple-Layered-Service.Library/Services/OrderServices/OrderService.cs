...
public class OrderService : IOrderService
{
    ...
    public async Task<PagedResult<ListAllOrderDto>> GetAllAsync(Pagination pagination)
    {
        try
        {
            var cacheKey = CacheHelpers.GenerateCacheKey("orders_cache", pagination);

            var cachedResult = await _unitOfWork.GetFromCacheAsync<PagedResult<ListAllOrderDto>>(cacheKey);
            if (cachedResult is not null)
            {
                _logger.LogInformation("Siparişler cache'den getirildi. Sayfa: {Page}, Boyut: {Size}",
